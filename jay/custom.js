...
var mof = document.getElementById("myM");
...
//function Modl(){
//	var nk = ['https://mysportdab.com','https://naijatweet.ng','https://www.faithgateway.com']
//	$('.sgs').attr('src',choose(nk));
//	$('.modl').css('display', 'block');
...
function checkCook(){
	var user = getCook('username');
	if (user == ""){
		
		 setTimeout(Modl, 15000);
			setCook("username",'junno');
	}
...
jQuery(document).ready(function ($) {
   ...
   setTimeout(NUnReadU(), 5000);
  checkCook();
   
    $('.cgk').click(function () {
                    setTimeout(NotifyUnReadUser(), 5000);
					
                });
				$('.cls').click(function(){
			$('.modl').css('display', 'none');
			('.sgs').attr('class', '');
	});
...
jQuery(document).ready(function(){
	...
	jQuery('.search-filter').css({'margin-top':-60+'px'});
